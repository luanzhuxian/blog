...
`commit-id`可以是commit对象对应的ID，也可以是HEAD，分支名，tag等。
...
实用技巧：当手头工作没有完成时，先把工作现场`git stash`一下，然后去修复bug，修复后，再使用`git stash pop`，回到工作现场.
</blockquote>
...
```
- 运行以下命令，拉去合并到本地，在本地编辑内容前必须执行pull操作同步别人的远端库（这样避免冲突）
```
