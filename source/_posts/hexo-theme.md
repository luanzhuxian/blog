...
![avatar](/images/blog/hexo-theme/qiniu-space.png)

之后在该`存储空间 > 内容管理 > 上传文件`上传图片：

![avatar](/images/blog/hexo-theme/qiniu-upload.png)
...
![avatar](/images/blog/hexo-theme/qiniu-list.png)

另外在图片样式下可以对图片进行处理，比如缩放、裁剪、增加水印、设置输出格式等。使用时只要在图片外链后加上设置的样式分隔符和样式名称，即可得到处理后的图片。  

![avatar](/images/blog/hexo-theme/qiniu-style.png)
